  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            One barcode per stone ({lot.products.length} total). Each scans back to that stone.
          </p>
        </div>
        <PrintLabelButton />
      </div>
      ...
      {lot.products.length === 0 ? (
        <p className="text-sm text-zinc-500">No stones in this lot yet.</p>
      ) : (
        <div className="print-labels flex flex-wrap gap-3">
          {lot.products.map((p) => (
            <BarcodeLabel key={p.id} sku={p.sku} name={lot.lotNumber} caratWeight={p.caratWeight} />
          ))}
        </div>
      )}
    </div>
